...
class GithubRepositoriesCache @Inject constructor(private val db: GithubDatabase) : IGithubRepositoriesCache {
    ...
    override fun putUserRepos(user: GithubUser, repositories: List<GithubRepository>): Completable {
        return Completable.fromAction {
            val roomUser = db.userDao.findByLogin(user.login.orEmpty()) ?: error("don't found user in cache")
            val roomRepos = repositories.map {
                RoomGithubRepository(it.id, it.name.orEmpty(), it.fullName.orEmpty(), it.description.orEmpty(), roomUser.id)
            }
